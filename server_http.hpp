#ifndef HTTPSERVER_SERVER_HTTP_HPP
#define HTTPSERVER_SERVER_HTTP_HPP
...
/// forward class declaration for friend declaration.
template<class socket_type>
class Server;

/**
 * a base class to be inheriated by Server.
 * Note that there are several classes in it.
 * */
template<class socket_type>
class ServerBase {
protected:
...
public:
	/**
	 * A public sub-class of ServerBase.
	 * */
	...
	private:
		...
		void send_from_queue() {
			...
			strand.post([self]() {
				asio::async_write(*self->session->connection->socket, *self->send_queue.begin()->first,
													self->strand.wrap([self](const error_code &ec, std::size_t /*bytes_transferred*/) {
	...
	public:
		Response(std::shared_ptr<Session> session_, long timeout_content) noexcept : std::ostream(nullptr),
		...
		/**
		 * All kinds of convenient write methods.
		 * */
		...
		void write(StatusCode status_code, string_view content,
							 const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
			*this << "HTTP/1.1 " << SimpleWeb::status_code(status_code) << "\r\n";
		...
		/// Convenience function for writing status line, header fields, and content
		void write(StatusCode status_code, std::istream &content,
							 const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
			*this << "HTTP/1.1 " << SimpleWeb::status_code(status_code) << "\r\n";
		...
		/// TODO: keep filling public functions, but go to set up connection class.
	}; // class Response.

	/**
	 * Content is just a data wrapper.
	 * */
	/**
 	 * A public sub-class of ServerBase.
 	 * */
	...
	/**
	 * A wrapper for all kinds of data fields that make up of Request class.
	 * */
	/**
	 * A public sub-class of ServerBase.
	 * */
...
protected:
	/**
	 * A protected sub-class of ServerBase.
	 *
	 * There is a socket object inside connection.
	 * */
	class Connection : public std::enable_shared_from_this<Connection> {
	public:
		template<typename... Args>
		Connection(std::shared_ptr<ScopeRunner> handler_runner_, Args &&... args) noexcept : handler_runner(
				std::move(handler_runner_)), socket(new socket_type(std::forward<Args>(args)...)) {}
	...
	class Session {
	public:
		Session(std::size_t max_request_streambuf_size, std::shared_ptr<Connection> connection_) noexcept : connection(
...
private:
	/**
	 * A private sub-class of ServerBase.
	 * I don't know what does regex means.
	 * */
...
public:
	...
	std::map<regex_orderable, std::map<std::string, std::function<void(
...
protected:
	...
	ServerBase(unsigned short port) noexcept : config(port), connections(new std::unordered_set<Connection *>()),
																						 connections_mutex(new std::mutex()), handler_runner(new ScopeRunner()) {}
	...
	/**
	 * set up connection first, and then use socket to send data.
	 * */
	template<typename... Args>
	...
	/**
	 * Three low methods used to read data chunks.
	 * */
		...
		asio::async_read_until(*session->connection->socket, session->request->streambuf, "\r\n\r\n",
													 [this, session](const error_code &ec, std::size_t bytes_transferred) {
														 ...
														 if (!ec) {
															 ...
															 if (!RequestMessage::parse(session->request->content, session->request->method,
															 ...
															 if (header_it != session->request->header.end()) {
																 ...
																 if (content_length > num_additional_bytes) {
																	 ...
																	 asio::async_read(*session->connection->socket, session->request->streambuf,
																	 ...
																	 this->find_resource(session);
															 } else if ((header_it = session->request->header.find("Transfer-Encoding")) !=
																					session->request->header.end() && header_it->second == "chunked") {
	...
	void find_resource(const std::shared_ptr<Session> &session) {
		...
		for (auto &regex_method : resource) {
			/**
			 * regex_method 给出了某一个正则表达式所link到的一个map，这个map从一个字符串映射到一个function.
			 * 注意这里的 session->request->method 是一个session->request 内部的 string public member.
			 *
			 * request 当中还有一个叫 path 的 public member string.
			 *
			 * 简而言之就是 request 有好多种类型，每个 request 的类型由两个字符串唯一标示，一个是 path 另一个是 method.
			 *
			 * 其中 method 我懂，就是 GET POST HEAD DELETE 一类，
			 * 但是另一个 path 我就不太清楚了，我要好好找一找。
			 * */
//			std::cout << "session request method: " << session->request->method << std::endl;
//			std::cout << "session request path: " << session->request->path << std::endl;
//			std::cout << "regex_method.first" << regex_method.first << std::endl;
//			std::cout.flush();
																							...
																							[this](Response *response_ptr) {
																								...
																								response->send_on_delete([this, response](const error_code &ec) {
																									if (!ec) {
																										...
																										auto range = response->session->request->header.equal_range(
																												"Connection");
...
#endif //HTTPSERVER_SERVER_HTTP_HPP
