#ifndef HTTPSERVER_UTILITY_HPP
#define HTTPSERVER_UTILITY_HPP
...
/**
 * Some boring function with strange equal and hash method.
 * */
...
/**
 * An unordered string to string map, with case insensitive hash and equal.
 * */
using CaseInsensitiveMultimap = std::unordered_multimap<std::string, std::string, CaseInsensitiveHash, CaseInsensitiveEqual>;

/**
 * Using a strange encode, decode protocol named percent, I don't know what it is.
 * Oh, I think I know what percent encoding means.
 * */
...
public:
	...
	static bool
	parse(std::istream &stream, std::string &method, std::string &path, std::string &query_string, std::string &version,
...
public:
	...
	static bool parse(std::istream &stream, std::string &version, std::string &status_code,
										CaseInsensitiveMultimap &header) noexcept {
		std::string line;
...
/**
 * I don't know what the following stuffs are.
 * */
#ifdef __SSE2__
