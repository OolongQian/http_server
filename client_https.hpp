#ifndef HTTPSERVER_CLIENT_HTTPS_HPP
#define HTTPSERVER_CLIENT_HTTPS_HPP
...
public:
	/**
	 * Based on the input verify_certificate and cert_file, private_key_file and verify_file (all those config files)
	 * to setup network connection context.
	 * */
...
protected:
	...
	void handshake(const std::shared_ptr<Session> &session) {
		...
		session->connection->set_timeout(this->config.timeout_connect);
		/**
		 * It seems that handshake details are handled by socket->async_handshake.
		 * This function is a process wrapper.
		 * */
...
#endif //HTTPSERVER_CLIENT_HTTPS_HPP
